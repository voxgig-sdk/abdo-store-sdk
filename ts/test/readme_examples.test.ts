...
//     (\`new AbdoStoreSDK(...)\` / \`AbdoStoreSDK.test(...)\` become
//     \`AbdoStoreSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"account":{"test01":{"id":"test01"}},"order":{"test01":{"id":"test01"}},"service":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AbdoStoreSDK(...)\` and any
// \`AbdoStoreSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AbdoStoreSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AbdoStoreSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
