...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.ABDOSTORE_TEST_LIVE ||
    'TRUE' === process.env.ABDOSTORE_TEST_OVERRIDE
  ) {
  ...
  m.ABDOSTORE_TEST_EXPLAIN = process.env.ABDOSTORE_TEST_EXPLAIN || m.ABDOSTORE_TEST_EXPLAIN
