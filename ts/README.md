# AbdoStore TypeScript SDK



The TypeScript SDK for the AbdoStore API — a type-safe, entity-oriented client with full async/await support.
...
```ts
const client = new AbdoStoreSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `AbdoStoreSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `AbdoStoreSDK.test(testopts?, sdkopts?)` | `AbdoStoreSDK` | Create a test-mode client. |
...
| `client` | `client(): AbdoStoreSDK` | Return the parent SDK client. |
...
```ts
const order = await client.Order().load({ id: 1 })
```
...
abdo-store/
├── src/
│   ├── AbdoStoreSDK.ts        # Main SDK class
