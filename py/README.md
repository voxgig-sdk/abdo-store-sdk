# AbdoStore Python SDK

The Python SDK for the AbdoStore API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Account(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.AbdoStore(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── abdostore_sdk.py         -- Main SDK module
