...
MCP server exposing the AbdoStore SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `abdo-store_list` | `entity`, optional `query` map | First page of records as JSON |
| `abdo-store_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `abdo-store_list` and
`abdo-store_load` tools become available in new sessions.
