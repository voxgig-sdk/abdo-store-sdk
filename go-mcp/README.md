...
[MCP](https://modelcontextprotocol.io) server exposing the AbdoStore SDK as
two agent tools — `abdo-store_list` and `abdo-store_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/abdo-store-mcp)
make build

# 2. Provide credentials via the environment
export ABDO_STORE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user abdo-store \
  -- /absolute/path/to/abdo-store-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./abdo-store-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// abdo-store_list: first page of records
{ "entity": "service" }
{ "entity": "service", "query": { } }

// abdo-store_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/abdo-store-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user abdo-store \
     -- "$PWD"/dist/*/abdo-store-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `abdo-store_list` and `abdo-store_load` tools now appear
   in new sessions. Ask the agent to *"list service using abdo-store"*
   and it calls `abdo-store_list` with `{"entity":"service"}`.
...
```sh
export ABDO_STORE_APIKEY=sk_live_xxx            # API key
export ABDO_STORE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `abdo-store_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `abdo-store_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ABDO_STORE_APIKEY` | API key sent with every request. |
| `ABDO_STORE_BASE` | Optional override of the API base URL. |
...
```sh
./abdo-store-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"abdo-store_load","arguments":{"entity":"account","query":{"id":1}}}}'
```
