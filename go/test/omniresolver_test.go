...
import (
	...
	omni "github.com/voxgig-sdk/abdo-store-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AbdoStoreError); is {
				out := map[string]any{
					"name":    "AbdoStoreError",
...
func omniCtx(arg any, client *sdk.AbdoStoreSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AbdoStoreSDK); is {
			client = live
