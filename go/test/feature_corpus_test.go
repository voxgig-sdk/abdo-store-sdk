...
//
// NewAbdoStoreSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.AbdoStoreSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewAbdoStoreSDK(opts)
}
...
func fcCandidates(client *sdk.AbdoStoreSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.AbdoStoreSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.AbdoStoreSDK, name string) any {
