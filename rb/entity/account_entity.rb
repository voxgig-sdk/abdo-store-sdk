...
class AccountEntity
  ...
  def make
    opts = @_entopts.dup
    AccountEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AbdoStoreHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Account, Hash] the current Account data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AbdoStoreHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Account fields)
  def match_get
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AbdoStoreHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
