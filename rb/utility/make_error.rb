...
module AbdoStoreUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = AbdoStoreContext.new({}, nil)
    end
    op = ctx.op || AbdoStoreOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || AbdoStoreResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(AbdoStoreError) ? err.msg : err.to_s
    ...
    sdk_err = AbdoStoreError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(AbdoStoreError)
