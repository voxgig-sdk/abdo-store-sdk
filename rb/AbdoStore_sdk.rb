...
class AbdoStoreSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AbdoStoreHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AbdoStoreHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AbdoStoreFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AbdoStoreUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = AbdoStoreHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AbdoStoreHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = AbdoStoreHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AbdoStoreError.new(
        "graphql_error", "AbdoStoreSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
