...
local AbdoStoreSDK = {}
AbdoStoreSDK.__index = AbdoStoreSDK
...
AbdoStoreSDK._make_feature = _make_feature


function AbdoStoreSDK.new(options)
  local self = setmetatable({}, AbdoStoreSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function AbdoStoreSDK:options_map()
...
function AbdoStoreSDK:get_utility()
  return Utility.copy(self._utility)
end


function AbdoStoreSDK:get_root_ctx()
  return self._rootctx
end


function AbdoStoreSDK:prepare(fetchargs)
...
function AbdoStoreSDK:direct(fetchargs)
...
-- by passing it.
function AbdoStoreSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function AbdoStoreSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "AbdoStoreSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function AbdoStoreSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = AbdoStoreSDK.new(sdkopts)
  sdk.mode = "test"
