...
The Lua SDK for the AbdoStore API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Account(nil):load({ id = "example_id" }, nil)
...
local result, err = client:AbdoStore(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── abdo-store_sdk.lua    -- Main SDK module
...
The main module (`abdo-store_sdk`) exports the SDK constructor
