...
describe("AccountEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set ABDOSTORE_TEST_ACCOUNT_ENTID JSON to run live")
      return
    end
...
function account_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("ABDOSTORE_TEST_ACCOUNT_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["ABDOSTORE_TEST_ACCOUNT_ENTID"])
  if idmap_resolved == nil then
    idmap_resolved = helpers.to_map(idmap)
  end

  if env["ABDOSTORE_TEST_LIVE"] == "TRUE" then
