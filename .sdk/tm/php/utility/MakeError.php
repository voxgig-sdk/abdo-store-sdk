...
class AbdoStoreMakeError
{
    public static function call(?AbdoStoreContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new AbdoStoreContext([], null);
        }
        $op = $ctx->op ?? new AbdoStoreOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new AbdoStoreResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof AbdoStoreError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new AbdoStoreError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof AbdoStoreError) {
