...
class FtCtrl extends AbdoStoreControl
{
...
class FtClient
{
    ...
    private AbdoStoreUtility $utility;

    public function __construct(AbdoStoreUtility $utility, array $options)
    {
    ...
    public function get_utility(): AbdoStoreUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AbdoStoreContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AbdoStoreContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AbdoStoreContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AbdoStoreConfig::make_feature($name);
    ...
    private static function buildUrl(AbdoStoreSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AbdoStoreError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AbdoStoreResponse($fetched);
        }
    ...
    private function fail(AbdoStoreContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AbdoStoreContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
