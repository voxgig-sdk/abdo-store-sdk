...
module AbdoStoreUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AbdoStoreResponse.new({})
    ...
    if fetch_err
      ...
      response = AbdoStoreResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AbdoStoreResponse.new(fetched)
    else
