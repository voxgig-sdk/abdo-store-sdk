...
boru-driven command-line client **and** interactive REPL for the AbdoStore
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/abdo-store-cli)
make build
...
./abdo-store-cli load 1 account            # {id:1} shorthand
./abdo-store-cli load '{id:1}' account       # explicit match map

# 5. Override the API base URL for a single call
ABDO_STORE_BASE=https://api.example.com ./abdo-store-cli load 1 account

# 6. No arguments -> interactive REPL
./abdo-store-cli
abdo-store> load 1 account
abdo-store> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/abdo-store-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/abdo-store-cli load 1 account
   ```
...
```sh
./abdo-store-cli load 1 account          # scalar shorthand for {id:1}
./abdo-store-cli load '{id:1}' account     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `abdo-store>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/abdo-store-cli`. |
...
(`./abdo-store-cli <expr>`) and interactively (the REPL), and expressions compose the
