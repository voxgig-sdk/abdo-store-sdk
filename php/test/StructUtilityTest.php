...
// AbdoStore SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = AbdoStoreOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(AbdoStoreOmni::structfix($now, Struct::undef()))
                        !== json_encode(AbdoStoreOmni::structfix($val, Struct::undef()))) {
