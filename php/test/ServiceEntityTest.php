...
class ServiceEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = AbdoStoreSDK::test($seed, null);
        $seen = iterator_to_array($base->Service(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = AbdoStoreConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = AbdoStoreSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ABDO_STORE_TEST_SERVICE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function service_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("ABDO_STORE_TEST_SERVICE_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["ABDO_STORE_TEST_SERVICE_ENTID"]);
