...
{
    private static function utility(): AbdoStoreUtility
    {
        return new AbdoStoreUtility();
    }
    ...
    private static function ctx(array $over = []): AbdoStoreContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AbdoStoreContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AbdoStoreOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AbdoStoreError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AbdoStoreOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AbdoStoreMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AbdoStoreOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AbdoStoreMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AbdoStoreOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AbdoStoreMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AbdoStoreSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AbdoStoreMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AbdoStoreResponse([]),
            'result' => new AbdoStoreResult([]),
        ]);
        [, $err] = AbdoStoreMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AbdoStoreSpec([]),
            'response' => null,
            'result' => new AbdoStoreResult([]),
        ]);
        [, $err] = AbdoStoreMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AbdoStoreSpec(['step' => 's']),
            'response' => new AbdoStoreResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AbdoStoreResult([]),
        ]);
        [, $err] = AbdoStoreMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AbdoStoreSpec(['step' => 's']),
            'response' => new AbdoStoreResponse(self::resp(200, ['v' => 1])),
            'result' => new AbdoStoreResult([]),
        ]);
        [, $err] = AbdoStoreMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AbdoStoreSpec(['step' => 's']),
            'response' => new AbdoStoreResponse(self::resp(200, ['v' => 2])),
            'result' => new AbdoStoreResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AbdoStoreMakeResponse::call($ctx);
    ...
    {
        $preset = new AbdoStoreResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AbdoStoreSpec([]),
            'response' => new AbdoStoreResponse([]),
            'result' => new AbdoStoreResult([]),
        ...
        [$got, $err] = AbdoStoreMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AbdoStoreResult([])]);
        [, $err] = AbdoStoreMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AbdoStoreSpec([]), 'result' => null]);
        [, $err] = AbdoStoreMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AbdoStoreSpec(['step' => 's']),
            'result' => new AbdoStoreResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AbdoStoreOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AbdoStoreMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AbdoStoreSpec(['step' => 's']),
            'result' => new AbdoStoreResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AbdoStoreOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AbdoStoreMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AbdoStoreResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AbdoStoreSpec([]),
            'result' => new AbdoStoreResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AbdoStoreMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AbdoStoreMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AbdoStoreError('boom', 'boom');
        $utility->fetcher = function (AbdoStoreContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AbdoStoreSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AbdoStoreMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AbdoStoreContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AbdoStoreMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AbdoStoreResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AbdoStoreContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AbdoStoreSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AbdoStoreMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AbdoStoreResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AbdoStoreSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AbdoStoreMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AbdoStoreMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AbdoStoreMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AbdoStoreResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AbdoStoreDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AbdoStoreResult(['ok' => false])]);
        $this->expectException(AbdoStoreError::class);
        AbdoStoreDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AbdoStoreResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AbdoStoreMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AbdoStoreMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AbdoStoreBaseFeature();
        $b = new AbdoStoreBaseFeature();
        AbdoStoreFeatureAdd::call($ctx, $a);
        AbdoStoreFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AbdoStoreBaseFeature
    {
        $f = new AbdoStoreBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AbdoStoreFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AbdoStoreContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AbdoStoreSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AbdoStorePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AbdoStoreResponse(['headers' => null]),
            'result' => new AbdoStoreResult([]),
        ]);
        AbdoStoreResultHeaders::call($ctx);
