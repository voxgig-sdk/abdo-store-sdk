...
class AccountEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ABDO_STORE_TEST_ACCOUNT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $account_ref01_data = null;
        if (count($account_ref01_data_raw) > 0) {
...
function account_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("ABDO_STORE_TEST_ACCOUNT_ENTID");
