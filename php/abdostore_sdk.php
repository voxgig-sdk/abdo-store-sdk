...
class AbdoStoreSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = AbdoStoreHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = AbdoStoreHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!AbdoStoreFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, AbdoStoreFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return AbdoStoreUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = AbdoStoreHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = AbdoStoreHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = AbdoStoreHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new AbdoStoreError($op . "_allow",
                "AbdoStoreSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = AbdoStoreHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new AbdoStoreError("graphql_error",
                "AbdoStoreSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
