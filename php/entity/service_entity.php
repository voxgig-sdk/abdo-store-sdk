...
class ServiceEntity
{
    ...
    {
        if ($args) {
            $this->_data = AbdoStoreHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = AbdoStoreHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws AbdoStoreError on failure (item-5 convention).
     */
